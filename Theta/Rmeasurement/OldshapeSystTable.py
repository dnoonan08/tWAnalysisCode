...
if len(condorOutLines) == 0:
    ...
    for line in condorOutLines:
        ...
        if 'uncertainty due to' in line:
            if not TheoryFixed:
                uncertainties[line.split()[3][:-1]][0] = float(line.split()[-1])
            else:
                uncertainties[line.split()[3][:-1]][1] = float(line.split()[-1])
        if 'quadratically added uncertainty' in line:
            if not TheoryFixed:
                totalUnc[0] = float(line.split(':')[-1].split()[0])
            else:
                totalUnc[1] = float(line.split(':')[-1].split()[0])
        if TheoryFixed:
            if 'shifts (plus/minus/absolute max.)' in line:
                uncertainties[line.split()[0]][1] = float(line.split(',')[-1])/22.2
            if 'total additional uncertaitnty from fixed parameters:' in line:
                percUnc = float(line.split()[-1])/22.2
                totalUnc[1] = math.sqrt(totalUnc[1]*totalUnc[1] + percUnc*percUnc)
#         if TheoryFixed:
...
#     unc = uncertainties[i]
for unc in uncerts1:
    print '    %s & %.2f & %.2f \\\\' % (unc[2], unc[0]*22.2, unc[0])
#    print ' ',unc[2],'&',unc[0]*22.2,'&',unc[0],'\\\\'
print '  \hline'
print '    Total & %.2f & %.2f \\\\' % (totalUnc[0], totalUnc[0]/22.2)
print '  \hline'
print '  \end{tabular}'
print '\end{center}'
print '\end{table*}'
...
#     unc = uncertainties[i]
for unc in uncerts2:
    print '    %s & %.2f & %.2f \\\\' % (unc[2], unc[1]*22.2, unc[1])
#    print ' ',unc[2],'&',unc[1]*22.2,'&',unc[1],'\\\\'
print '    \hline'
print '    Total & %.2f & %.2f \\\\' % (totalUnc[1]*22.2, totalUnc[1])
