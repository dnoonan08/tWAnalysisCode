...
#                 rootFile=[rootfile for rootfile in os.listdir('.') if rootfile.endswith('.root')][0]
            ...
            elif 'model = get_model' in line:         
                linestart = line.split('model = get_model')[0]
                newline=line+linestart+'setmodel = set([])\n'
                if channel !='ee/emu/mumu': #For all channel combination, but the All Channel one:
                    if '_' not in channel: #IndividualChannels
                        newline=newline+"\n"+linestart+"#Channel %s ONLY:\n"%channel
                        #G.Benelli Jan 2013
                        #Need to add 1j1t to the signal region model name!!!!
#                         newline=newline+linestart+'setmodel.add("%s1j1t")\nsetmodel.add("%s2j1t")\nsetmodel.add("%s2j2t")\nmodel.restrict_to_observables(setmodel)\n'%(channel,channel,channel)
                        newline=newline+linestart+'setmodel.add("%s1j1tT")\n'%(channel)
                        newline=newline+linestart+'setmodel.add("%s1j1tTbar")\n'%(channel)
                        if use2j1t:
                            newline=newline+linestart+'setmodel.add("%s2j1t")\n'%(channel)
                        if use2j2t:
                            newline=newline+linestart+'setmodel.add("%s2j2t")\n'%(channel)
                        newline=newline+linestart+'model.restrict_to_observables(setmodel)\n'
                    else:
                        newline=newline+"\n"+linestart+"#Channels %s %s ONLY:\n"%tuple(channel.split('_'))
