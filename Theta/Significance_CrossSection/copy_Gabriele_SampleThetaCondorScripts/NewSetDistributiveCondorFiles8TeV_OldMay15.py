...
fullSubmissions = 250
emuSubmissions = 50
twoChanSubmissions = 75
otherSubmissions = 5

nToys = 1000000

# emuSubmissions = 75 #Number of jobs for emu and 3 channel combo
# twoChanSubmissions = 1 #Number of jobs for 2 channel combos
# otherSubmissions = 1 #Number of jobs for ee and mumu channels

# nToys = 400000  #Number of toys per job

# emuSubmissions = 4 #Number of jobs for emu and 3 channel combo
# twoChanSubmissions = 4 #Number of jobs for 2 channel combos
# otherSubmissions = 2 #Number of jobs for ee and mumu channels

# nToys = 250000  #Number of toys per job

# emuSubmissions = 1 #Number of jobs for emu and 3 channel combo
# twoChanSubmissions = 1 #Number of jobs for 2 channel combos
# otherSubmissions = 1 #Number of jobs for ee and mumu channels

# nToys = 1000  #Number of toys per job
...
#File for making the Signal plus background toys

# pythonSplusBCfg='analysisJochenSyst8TeV_SplusB.py'

# print "Copying the theta configuration template file %s from %s/%s"%(pythonSplusBCfg,templatesDir,pythonSplusBCfg)
# cmdOutput=subprocess.Popen('cp -pR %s/%s .'%(templatesDir,pythonSplusBCfg),shell=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT).stdout.read()
# print cmdOutput

# #Now given the theta python configuration file (that will use all the channels in the model)  generate the alternative configurations
# #to look into the single and pairs of channels:
# pythonSplusBCfgOriginalLines=open(pythonSplusBCfg,'r').readlines()
...
#     if channel == 'ee/emu/mumu':
#         pythonSplusBCfgNew=open(pythonSplusBCfg,'w')
#     else:
#         pythonSplusBCfgNew=open(pythonSplusBCfg.replace('.py','_'+channel+'.py'),'w')
#     for line in pythonSplusBCfgOriginalLines:
#         if 'ROOTFILEINPUT' in line:
#             print "Will automatically set all theta cfgs to use the root file found in this directory:"
#             rootFile=[rootfile for rootfile in os.listdir('.') if rootfile.endswith('.root')][0]
#             print "%s"%rootFile
...
#                     newline=newline+'model.restrict_to_observables(setmodel)\n'
#         elif 'NUMBEROFRUNS' in line:
#             newline = line.replace('NUMBEROFRUNS',str(maxSubmissions)).replace('TOYSPERRUN',str(nToys))
#         elif 'TOYSPERRUN' in line:
#             newline = line.replace('TOYSPERRUN',str(nToys))
#         elif doQ2ShapeFix and 'Q2ShapeFixed=' in line:
#             newline = line.replace('=False','=True')
...
#         else:
#             newline=line
#         pythonSplusBCfgNew.write(newline)
