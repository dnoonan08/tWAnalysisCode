...
for fix_uncertainties in [['DRDS', 'Matching', 'Q2', 'TopMass', 'TopPt']]:
    ...
    if do_significance:
        ...
        exp05, exp_low1, exp_up1, obs = discovery(model, nuisance_constraint = fixed_dist, maxit = NUMBEROFRUNS, n = TOYSPERRUN, ts_method = TESTSTAT, n_expected = 10000)
#        exp05, exp_low1, exp_up1, obs = discovery(model, nuisance_constraint = fixed_dist, maxit = NUMBEROFRUNS, n = TOYSPERRUN, ts_method = derll, n_expected = 10000)
#        exp05, exp_low1, exp_up1, obs = discovery(model, nuisance_constraint = fixed_dist, maxit = NUMBEROFRUNS, n = TOYSPERRUN, ts_method = deltanll)
        print "significances (all '+-' uncertainties are due to limited number of background-only toys):"
        print exp05, exp_low1, exp_up1
        print "expected significance (1sigma low, 1sigma high): %f +- %f"%exp05,"  (%f +- %f"%exp_low1,", %f +- %f)"%exp_up1
        print "observed significance: %f +- %f" % obs
