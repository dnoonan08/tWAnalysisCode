...
plotInfo = [['BDT',5,0,5,'BDT discriminant']]
...
for mode in range(3):
    ...
    for i in range(len(fileList)):
        ...
        for plot in plotInfo:
            ...
            for bin in range(5):
                Histos[plot[0]][0].GetXaxis().SetBinLabel(bin+1,binNames[bin])
                Histos[plot[0]][1][0].GetXaxis().SetBinLabel(bin+1,binNames[bin])
                Histos[plot[0]][1][1].GetXaxis().SetBinLabel(bin+1,binNames[bin])
                Histos[plot[0]][2].GetXaxis().SetBinLabel(bin+1,binNames[bin])
...
#                 _btagSF = event.weightBtagSF
            ...
            HistoLists[mode][i]['BDT'][0].Fill(                    _ptjet                    , _weight )
...
for mode in range(3):
    ...
    for plot in plotInfo:
        ...
        for s in range(len(systList)):
            ...
            if not os.path.exists("SystematicsPlots/"+specialName):
                command = "mkdir SystematicsPlots/"+specialName
                os.system(command)
            if not os.path.exists("SystematicsPlots/"+specialName + region):
                command = "mkdir SystematicsPlots/"+specialName+region
                os.system(command)
            
            channel = "_" + ChanName[mode]

        
            c1.SaveAs("SystematicsPlots/"+specialName+region+"/"+plot[0]+"_tW_"+region+channel+runs+systList[s]+".png")
    ...
    for plot in plotInfo:
        ...
        for s in range(len(systList)):
            ...
            c1.SaveAs("SystematicsPlots/"+specialName+region+"/"+plot[0]+"_tt_"+region+channel+runs+systList[s]+".png")
