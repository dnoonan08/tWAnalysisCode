...
specialName = 'InputVariableBins/'+BDT+'/'
...
# plotInfo = [['BDTNloosejet0',50,-1.01,1.01,'BDT Discriminant - No Loose Jets','Events'],
#             ['BDTNloosejet1',50,-1.01,1.01,'BDT Discriminant - One Loose Jet','Events'],
#             ['BDTNloosejet2plus',50,-1.01,1.01,'BDT Discriminant - Two Loose Jets','Events'],
#             ['BDTNbtaggedloosejet0',50,-1.01,1.01,'BDT Discriminant - No Btagged Loose Jets','Events'],
#             ['BDTNbtaggedloosejet1plus',50,-1.01,1.01,'BDT Discriminant - One Btagged Loose Jet','Events'],
#             ]

plotInfo = [['BDT',35,-0.4,0.3,'BDT Discriminant - No Loose Jets','Events'],
            ['BDTNloosejet0',35,-0.4,0.3,'BDT Discriminant - No Loose Jets','Events'],
            ['BDTNloosejet1',35,-0.4,0.3,'BDT Discriminant - One Loose Jet','Events'],
            ['BDTNloosejet2plus',35,-0.4,0.3,'BDT Discriminant - Two Loose Jets','Events'],
            ['BDTNbtaggedloosejet0',35,-0.4,0.3,'BDT Discriminant - No Btagged Loose Jets','Events'],
            ['BDTNbtaggedloosejet1plus',35,-0.4,0.3,'BDT Discriminant - One Btagged Loose Jet','Events'],
            ['BDTptsys_0_30',35,-0.4,0.3,'BDT Discriminant - ptsys less than 30','Events'],
            ['BDTptsys_30_60',35,-0.4,0.3,'BDT Discriminant - ptsys 30 to 60','Events'],
            ['BDTptsys_60plus',35,-0.4,0.3,'BDT Discriminant - ptsys greater than 60','Events'],
            ]
    ...
    for i in range(len(fileList)):
        ...
        if fileList[i] == 'DATA':
            if RunA:
                ...
                BDTtree.Add('../tmvaFiles/'+vFolder+'/'+BDT+'/Data_'+DataChannel[mode]+"_Run2012A_Output.root")
            if RunB:
                tree.Add('../tmvaFiles/'+vFolder+'/Data_'+DataChannel[mode]+'_Run2012B.root')
                BDTtree.Add('../tmvaFiles/'+vFolder+'/'+BDT+'/Data_'+DataChannel[mode]+"_Run2012B_Output.root")
            if RunC:
                tree.Add('../tmvaFiles/'+vFolder+'/Data_'+DataChannel[mode]+'_Run2012C.root')
                BDTtree.Add('../tmvaFiles/'+vFolder+'/'+BDT+'/Data_'+DataChannel[mode]+"_Run2012C_Output.root")
        ...
        for event,bdtevt in itertools.izip(tree,BDTtree):
            ...
            if _NlooseJet20 == 0:
                HistoLists[mode][i]['BDTNloosejet0'].Fill(   _BDT   , _weight )
                HistoLists[-1][i]['BDTNloosejet0'].Fill(   _BDT   , _weight )
            if _NlooseJet20 == 1:
                HistoLists[mode][i]['BDTNloosejet1'].Fill(   _BDT   , _weight )
                HistoLists[-1][i]['BDTNloosejet1'].Fill(   _BDT   , _weight )
            if _NlooseJet20 > 1:
                HistoLists[mode][i]['BDTNloosejet2plus'].Fill(   _BDT   , _weight )
                HistoLists[-1][i]['BDTNloosejet2plus'].Fill(   _BDT   , _weight )
            if _NbtaggedlooseJet20 == 0:
                HistoLists[mode][i]['BDTNbtaggedloosejet0'].Fill(   _BDT   , _weight )
                HistoLists[-1][i]['BDTNbtaggedloosejet0'].Fill(   _BDT   , _weight )
            if _NbtaggedlooseJet20 > 0:
                HistoLists[mode][i]['BDTNbtaggedloosejet1plus'].Fill(   _BDT   , _weight )
                HistoLists[-1][i]['BDTNbtaggedloosejet1plus'].Fill(   _BDT   , _weight )
            if _ptsys < 30:
                HistoLists[mode][i]['BDTptsys_0_30'].Fill(   _BDT   , _weight )
                HistoLists[-1][i]['BDTptsys_0_30'].Fill(   _BDT   , _weight )
            elif _ptsys < 60:
                HistoLists[mode][i]['BDTptsys_30_60'].Fill(   _BDT   , _weight )
                HistoLists[-1][i]['BDTptsys_30_60'].Fill(   _BDT   , _weight )
            else:
                HistoLists[mode][i]['BDTptsys_60plus'].Fill(   _BDT   , _weight )
                HistoLists[-1][i]['BDTptsys_60plus'].Fill(   _BDT   , _weight )
...
leg.SetBorderSize(1)
leg.AddEntry(HistoLists[0][-1]['BDTNloosejet0'], "Data", "p")
leg.AddEntry(HistoLists[0][0]['BDTNloosejet0'], "tW", "f")
leg.AddEntry(HistoLists[0][1]['BDTNloosejet0'], "t#bar{t}", "f")
leg.AddEntry(HistoLists[0][4]['BDTNloosejet0'], "Z/#gamma*+jets", "f")
leg.AddEntry(HistoLists[0][2]['BDTNloosejet0'], "Other", "f")
...
#            if region == 'tree1j1tNomllMetCut' or '1j1tZpeak' in region:
            if 'tree1j1tNomllMetCut' or '1j1tZpeak' in region:
                regionTemp = '1j1t'
            if '1j1tZpeak' in region:
                regionTemp = '1j1t'
