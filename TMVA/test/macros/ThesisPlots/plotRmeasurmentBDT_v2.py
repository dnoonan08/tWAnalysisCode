...
file = TFile('../HistogramFile/AdaBoostDefault_NewTests_BDTRmeasurement_v2_New.root','r')

#KU Colors
colortw = kOrange-2
colortbarw = kOrange+7
colortt = kRed+1
colorother = kGreen+2
colorzjet = kAzure-6
...
l_tbarw   = TH1F('b','',1,0,1)
l_tbarw.SetFillColor(Colors[1])
l_tt   = TH1F('c','',1,0,1)
l_tt.SetFillColor(Colors[2])
l_zjet = TH1F('d','',1,0,1)
l_zjet.SetFillColor(Colors[5])
l_oth  = TH1F('e','',1,0,1)
l_oth.SetFillColor(Colors[3])
l_syst = TH1F('f','',1,0,1)
...
leg.AddEntry(l_tbarw  , "#lower[-0.1]{#it{#bar{t}}}#lower[-0.1]{#it{W^{+}}}", "f")
# leg.AddEntry(l_tbarw  , "#lower[-0.1]{#it{#bar{t}W}}", "f")
...
SampleSysts = {'TWChannel_T' : Systs[:]+ExtraTWSysts[:],
               'TWChannel_Tbar' : Systs[:]+ExtraTWSysts[:],
...
#            error.SetFillColor(kGray+1)
            ...
            histTot2.Add(hists[3])
            histTot2.Add(hists[5])
...
#             lineAtOne.Draw("e same")
            ...
            if not os.path.exists("plots3/"):
                command = "mkdir plots3/"
                os.system(command)
            if not os.path.exists("plots3/"+BDT):
                command = "mkdir plots3/"+BDT+"/"
                os.system(command)
            if not os.path.exists("plots3/"+BDT+"/"+reg):
                command = "mkdir plots3/"+BDT+"/"+reg
                os.system(command)

            c1.SaveAs("plots3/"+BDT+"/"+reg+"/"+plot+"_"+reg+"_"+mode+".pdf")
            c1.SaveAs("plots3/"+BDT+"/"+reg+"/"+plot+"_"+reg+"_"+mode+".png")
...
#            t1.Clear()
        ...
        histTot2.Add(regionTotals[0][3])
        histTot2.Add(regionTotals[0][5])
...
#         labelcms.AddText(ksLabel)
        ...
        hStack = THStack(plot,plot)
        hStack.Add(regionTotals[0][3])
        hStack.Add(regionTotals[0][5])
...
#        t2 = TPad("t2", "t1", 0.,.15,1.,1.)
        ...
        c1.SaveAs("plots3/"+BDT+"/"+reg+"/"+plot+"_"+reg+".pdf")
        c1.SaveAs("plots3/"+BDT+"/"+reg+"/"+plot+"_"+reg+".png")
