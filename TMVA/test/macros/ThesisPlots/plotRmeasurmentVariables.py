...
if RunD: file = TFile('../../HistogramFile/FullVariablePlots_RunD.root','r')
elif RunABC: file = TFile('../../HistogramFile/FullVariablePlots_RunABC.root','r')
else: file = TFile('../HistogramFile/FullVariablePlots_New_2.root','r')
...
labelcms = TPaveText(0.1,0.9,0.9,.98,"NDCBR")
...
plots =  [
    ...
    'Toplep_lepPtRelJet',
    'Toplep_jetPtRelLep',
    'Toplep_lepPtRelJetSameLep',
    'Toplep_lepPtRelJetOtherLep',
    'Toplep_lepJetMt',
    'Toplep_lepJetCosTheta_boosted',
    'Wlep_lepJetPt',
    'Wlep_lepPt',
    'Wlep_lepJetDR',
    'Wlep_lepJetDPhi',
    ...
    'Wlep_lepPtRelJet',
    'Wlep_jetPtRelLep',
    'Wlep_lepPtRelJetSameLep',
    'Wlep_lepPtRelJetOtherLep',
    'Wlep_lepJetMt',
    'Wlep_lepJetCosTheta_boosted',    
    ]
...
leg = TLegend(0.68,0.55,0.94,0.89)
...
leg2 = TLegend(0.13,0.55,0.94,0.89)
leg2.SetFillColor(kWhite)
leg2.SetBorderSize(1)
leg2.AddEntry(l_data, "Data", "p")
leg2.AddEntry(l_tw  , "#it{tW}", "f")
leg2.AddEntry(l_tt  , "#lower[-0.1]{#it{t#bar{t}}}", "f")
leg2.AddEntry(l_zjet, "#lower[0.18]{#it{Z/#gamma*}+jets}", "f")
leg2.AddEntry(l_oth , "Other", "f")
leg2.AddEntry(l_syst, "Uncertainty", "f")
...
plotInfo = {'ptjet':['p_{T} leading jet [GeV]','Events / 10 GeV'],
            ...
            'ptsys':['p_{T}^{sys} [GeV]','Events / 4 GeV'],
#            'ptsys':['p_{T} system [GeV]','Events / 4 GeV'],
            ...
            'lepJetPt':['P_{T} jet & lepton','Events'],
            'lepPt':['P_{T} lepton','Events'],
            'lepJetDR':['#Delta R(jet,lepton)','Events'],
            'lepJetDPhi':['#Delta #phi (jet,lepton)','Events'],
            'lepJetDEta':['#Delta #eta (jet,lepton)','Events'],
            'lepJetM':['Mass of jet & lepton','Events'],
            'lepPtRelJet':['P_{T} lepton relative to jet','Events'],
            'jetPtRelLep':['P_{T} jet relative to lepton','Events'],
            'lepPtRelJetSameLep':['P_{T} lepton relative to jet & lep','Events'],
            'lepPtRelJetOtherLep':['P_{T} lepton relative to jet & other lep','Events'],
            'lepJetMt':['M_{T} jet & lepton','Events'],
            'lepJetCosTheta_boosted':['Cos #theta (lepton,jet)','Events'],

            'Toplep_lepJetPt':['P_{T} jet & lepton','Events'],
            'Toplep_lepPt':['P_{T} lepton','Events'],
            'Toplep_lepJetDR':['#Delta R(jet,lepton)','Events'],
            'Toplep_lepJetDPhi':['#Delta #phi (jet,lepton)','Events'],
            'Toplep_lepJetDEta':['#Delta #eta (jet,lepton)','Events'],
            'Toplep_lepJetM':['Mass of jet & lepton','Events'],
            'Toplep_lepPtRelJet':['P_{T} lepton relative to jet','Events'],
            'Toplep_jetPtRelLep':['P_{T} jet relative to lepton','Events'],
            'Toplep_lepPtRelJetSameLep':['P_{T} lepton relative to jet & lep','Events'],
            'Toplep_lepPtRelJetOtherLep':['P_{T} lepton relative to jet & other lep','Events'],
            'Toplep_lepJetMt':['M_{T} jet & lepton','Events'],
            'Toplep_lepJetCosTheta_boosted':['Cos #theta (lepton,jet)','Events'],

            'Wlep_lepJetPt':['P_{T} jet & lepton','Events'],
            'Wlep_lepPt':['P_{T} lepton','Events'],
            'Wlep_lepJetDR':['#Delta R(jet,lepton)','Events'],
            'Wlep_lepJetDPhi':['#Delta #phi (jet,lepton)','Events'],
            'Wlep_lepJetDEta':['#Delta #eta (jet,lepton)','Events'],
            'Wlep_lepJetM':['Mass of jet & lepton','Events'],
            'Wlep_lepPtRelJet':['P_{T} lepton relative to jet','Events'],
            'Wlep_jetPtRelLep':['P_{T} jet relative to lepton','Events'],
            'Wlep_lepPtRelJetSameLep':['P_{T} lepton relative to jet & lep','Events'],
            'Wlep_lepPtRelJetOtherLep':['P_{T} lepton relative to jet & other lep','Events'],
            'Wlep_lepJetMt':['M_{T} jet & lepton','Events'],
            'Wlep_lepJetCosTheta_boosted':['Cos #theta (lepton,jet)','Events'],
...
for plot in plots:
    if 'centralityJLL' in plot or 'lepJetDPhi' in plot:
        leg = TLegend(0.16,0.55,0.42,0.89)
    else:        
        leg = TLegend(0.68,0.55,0.94,0.89)
...
#             labelcms.AddText(reg + ChanLabels[mode])
            labelcms.Clear()
#            labelcms.AddText("CMS, #sqrt{s} = 8 TeV, " + lumiLabel);
            labelcms.AddText("CMS Preliminary, #sqrt{s} = 8 TeV, " + lumiLabel + ", " + reg + ChanLabels[mode]);
            labelcms.SetTextSize(0.042);

#             labelcms.AddText("#bf{#bf{CMS, #sqrt{s} = 8 TeV, " + lumiLabel + "}}");
#             labelcms.AddText("#bf{#bf{" + reg + ChanLabels[mode] + "}}")
...
#             dataRatio.SetMinimum(0.1)
#             dataRatio.SetMaximum(1.9)
...
#             dataRatio.GetXaxis().SetTitleOffset(.83)
...
#             oneLine = TH1F("line","",1,dataRatio.GetXaxis().GetXmin(),dataRatio.GetXaxis().GetXmax())
#             oneLine.SetBinContent(1,1)
#             oneLine.SetBinError(1,0)
#             oneLine.SetLineColor(kBlack)
#             oneLine.SetLineWidth(1)
...
#             oneLine.Draw("e same")
            ...
            hStack.Draw("histo")
            hStack.SetMaximum(max_*1.15)
            hStack.SetMinimum(0)
            if 'loosejetPt' in plot: hStack.GetXaxis().SetRangeUser(0,150)
            ...
            if not os.path.exists("plots2/"):
                ...
                os.system(command)
            if not os.path.exists("plots2/"+reg):
                command = "mkdir plots2/"+reg
                os.system(command)

            

            c1.SaveAs("plots2/"+reg+"/"+plot+"_"+reg+"_"+mode+extraRunInfo+".pdf")
            c1.SaveAs("plots2/"+reg+"/"+plot+"_"+reg+"_"+mode+extraRunInfo+".png")
...
#         labelcms.Clear()
# #         labelcms.AddText("#bf{#bf{CMS, #sqrt{s} = 8 TeV, " + lumiLabel + "}}");
# #         labelcms.AddText("#bf{#bf{" + reg + ChanLabels[' '] + "}}")
#         labelcms.AddText("CMS, #sqrt{s} = 8 TeV, " + lumiLabel);
#         labelcms.AddText(reg +ChanLabels[' '])
        labelcms.Clear()
#        labelcms.AddText("CMS, #sqrt{s} = 8 TeV, " + lumiLabel);
        labelcms.AddText("CMS Preliminary, #sqrt{s} = 8 TeV, " + lumiLabel + ", " + reg + ChanLabels[' '])
...
#         oneLine.Draw("e same")
        ...
        hStack.Draw("histo")
        if 'Toplep_lepJetDPhi' in plot or 'Toplep_lepJetCosTheta_boosted' in plot:
        ...
        hStack.GetYaxis().SetTitleSize(0.07)
        hStack.GetYaxis().SetLabelSize(0.05)
        hStack.GetXaxis().SetTitleOffset(0.98)
        hStack.GetXaxis().SetTitleSize(0.06)
        hStack.GetXaxis().SetLabelSize(0.05)
...
#         dataRatio.GetXaxis().SetTitleOffset(.83)
        ...
        c1.SaveAs("plots2/"+reg+"/"+plot+"_"+reg+extraRunInfo+".pdf")
        c1.SaveAs("plots2/"+reg+"/"+plot+"_"+reg+extraRunInfo+".png")
