...
#                 directory = 'v11_MET50'
            ...
            if file == 'DATA':
                ...
                for b in range(HistoLists[chan][reg][fileHistoInput[fileNum]][0].GetNbinsX()):
                    binTot = HistoLists[chan][reg][0][0].GetBinContent(b+1) + HistoLists[chan][reg][1][0].GetBinContent(b+1) + HistoLists[chan][reg][2][0].GetBinContent(b+1)                
                    HistoLists[chan][reg][fileHistoInput[fileNum]][0].SetBinContent(b+1,round(binTot))
                    HistoLists[chan][reg][fileHistoInput[fileNum]][0].SetBinError(b+1,sqrt(round(binTot)))
...
#                    tree.Add('../tmvaFiles/'+directory+'/'+fileName)
        ...
        for i in range(HistoLists[chan][reg][0][0].GetNbinsX()):
            if DS_symm:
                ...
                HistoLists[chan][reg][0][1][-1][1].SetBinContent(i+1,HistoLists[chan][reg][0][0].GetBinContent(i+1)*0.995)
...
outFile = TFile('inputFiles/inputFileTheta_'+BDT+ExtraNames+'_FakeData.root','RECREATE')
