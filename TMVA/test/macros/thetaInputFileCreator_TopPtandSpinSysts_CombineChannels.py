...
BDT = 'AdaBoostDefault_NewTests_2'
...
for reg in regions:
    tw = TH1F(reg+ "__twdr","",nBins,limitLow,limitHigh)
    tw.Sumw2()
    tt = TH1F(reg+ "__tt","",nBins,limitLow,limitHigh)
    tt.Sumw2()
    other = TH1F(reg+ "__other","",nBins,limitLow,limitHigh)
    other.Sumw2()
    data = TH1F(reg+ "__DATA","",nBins,limitLow,limitHigh)
    ...
    for syst in Systs:
        twsystUp = TH1F(reg+ "__twdr__"+syst + "__plus","",nBins,limitLow,limitHigh)
        twsystDown = TH1F(reg+ "__twdr__"+syst + "__minus","",nBins,limitLow,limitHigh)
        tempsystTW = [twsystUp,twsystDown]
        ttbarsystUp = TH1F(reg+ "__tt__"+syst + "__plus","",nBins,limitLow,limitHigh)
        ttbarsystDown = TH1F(reg+ "__tt__"+syst + "__minus","",nBins,limitLow,limitHigh)
        tempsystTT = [ttbarsystUp,ttbarsystDown]
        othersystUp = TH1F(reg+ "__other__"+syst + "__plus","",nBins,limitLow,limitHigh)
        othersystDown = TH1F(reg+ "__other__"+syst + "__minus","",nBins,limitLow,limitHigh)
    ...
    if useDS:
        twsystUp = TH1F(reg+ "__twdr__DRDS__plus","",nBins,limitLow,limitHigh)
        twsystDown = TH1F(reg+ "__twdr__DRDS__minus","",nBins,limitLow,limitHigh)
        tempsystTW = [twsystUp,twsystDown]
        twsyst.append(tempsystTW)

    #ADDING IN ZJET SF HISTOS BY HAND
    if useZJetSF:
        zjetSFUp = TH1F(reg+ "__other__ZjetSF__plus","",nBins,limitLow,limitHigh)
        zjetSFDown = TH1F(reg+ "__other__ZjetSF__minus","",nBins,limitLow,limitHigh)
        ...
        for fileNum in range(len(fileList)):
            ...
            for event,var in itertools.izip(tree,vartree):
                ...
                if useZJetSF:
                    if fileHistoInput[fileNum]==2:
                        HistoLists[reg][fileHistoInput[fileNum]][1][-1][0].Fill(_BDT, _weightUp)
                        HistoLists[reg][fileHistoInput[fileNum]][1][-1][1].Fill(_BDT, _weightDown)
                ...
                if 'TTbar' in file:
                    if useTopPtReweight:
                        ...
                        HistoLists[reg][fileHistoInput[fileNum]][1][ExtraTTbarSysts.index('TopPt')-len(ExtraTTbarSysts)][0].Fill(_BDT, startWeight*(1.+2*_topPTSF))
                        HistoLists[reg][fileHistoInput[fileNum]][1][ExtraTTbarSysts.index('TopPt')-len(ExtraTTbarSysts)][1].Fill(_BDT, startWeight)
                    else:
                        _topPTSF =  var.weightTopPt - 1.                                        
                        HistoLists[reg][fileHistoInput[fileNum]][1][ExtraTTbarSysts.index('TopPt')-len(ExtraTTbarSysts)][0].Fill(_BDT, _weight*(1.+_topPTSF))
                        HistoLists[reg][fileHistoInput[fileNum]][1][ExtraTTbarSysts.index('TopPt')-len(ExtraTTbarSysts)][1].Fill(_BDT, _weight*(1.-_topPTSF))
                ...
                HistoLists[reg][fileHistoInput[fileNum]][0].Fill(_BDT, _weight)
            print
            #                HistoLists[reg][-1][0].Fill(_BDT, _weight)
            ...
            for s in range(len(SystList)):
                ...
                for event,var in itertools.izip(tree,vartree):
                    ...
                    HistoLists[reg][fileHistoInput[fileNum]][1][s][0].Fill(_BDT, _weight)
                ...
                for event,var in itertools.izip(tree,vartree):
                    ...
                    HistoLists[reg][fileHistoInput[fileNum]][1][s][1].Fill(_BDT, _weight)
            ...
            if "TWChannel" in file:
                ...
                for i in range(len(SpecSysts)):
                    ...
                    for event in tree:
                        ...
                        HistoLists[reg][0][1][histSpot[i]][systUpDown[i]].Fill(_BDT, _weight)
                    print
    
    
        for i in range(HistoLists[reg][0][0].GetNbinsX()):
            if DS_symm:
                HistoLists[reg][0][1][-1][1].SetBinContent(i+1, HistoLists[reg][0][0].GetBinContent(i+1)*2.0 - HistoLists[reg][0][1][-1][0].GetBinContent(i+1))
            else:
                HistoLists[reg][0][1][-1][1].SetBinContent(i+1,HistoLists[reg][0][0].GetBinContent(i+1)*0.995)
        
                #        HistoLists[reg][0][1][-1][1] = HistoLists[reg][0][0].Clone(ChanName[chan] + regions[reg] + "__twdr__DRDS__minus")
