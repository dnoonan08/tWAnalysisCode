...
#regions = ['ZpeakLepSel','1j1tZpeak']

for r in regions:
    if not os.path.exists('FullTestShapeOnly/'+r):
        command = 'mkdir FullTestShapeOnly/'+r
        os.system(command)
...
l_tw   = TH1F('b','',1,0,1)
l_tw.SetFillColor(kWhite)
l_tt   = TH1F('c','',1,0,1)
l_tt.SetFillColor(kRed+1)
l_zjet = TH1F('d','',1,0,1)
l_zjet.SetFillColor(kAzure-2)
l_oth  = TH1F('e','',1,0,1)
l_oth.SetFillColor(kGreen-3)
l_syst = TH1F('f','',1,0,1)
l_syst.SetFillColor(kBlack)
l_syst.SetFillStyle(3013)        
...
#            print plot, mode, reg
            ...
            for i in range(len(samples)):
                ...
                ScaleNominal = hists[i].Integral()

                systHistsUp = list()
                systHistsDown = list()
                systs = SampleSysts[sample]
                for s in systs:
                    s_up=file.Get(plot+sample+mode+reg+'_'+s+'Up')
                    s_up_Int = s_up.Integral()
                    if not s_up_Int == 0.:
                        s_up.Scale(ScaleNominal/s_up_Int)
                    systHistsUp.append(s_up)                    
                    if 'DRDS' in s:
                        systHistsDown.append(file.Get(plot+sample+mode+reg))
                    else:
                        s_down = file.Get(plot+sample+mode+reg+'_'+s+'Down')
                        s_down_Int = s_down.Integral()
                        if not s_down_Int == 0.:
                            s_down.Scale(ScaleNominal/s_down_Int)
                        systHistsDown.append(s_down)
            ...
            dataRatio.GetXaxis().SetLabelSize(0.1)
            if 'BDTRe' in plot:
                dataRatio.GetXaxis().SetLabelSize(0.18)
            ...
            c1.SaveAs("FullTestShapeOnly/"+reg+"/"+plot+"_"+reg+"_"+mode+".pdf")
        ...
        c1.SaveAs("FullTestShapeOnly/"+reg+"/"+plot+"_"+reg+".pdf")
