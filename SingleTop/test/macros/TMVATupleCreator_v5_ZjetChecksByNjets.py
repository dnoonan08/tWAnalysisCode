...
#fileVersion = 'v3_Rmeasurement'
fileVersion = 'ZpeakRegion_v2'
#fileVersion = 'TestDir'
...
gROOT.ProcessLine(
    ...
    Double_t   lepJetPt_PosLep;\
    Double_t   lepPt_PosLep;\
    Double_t   lepJetDR_PosLep;\
    Double_t   lepJetDPhi_PosLep;\
    Double_t   lepJetDEta_PosLep;\
    Double_t   lepJetM_PosLep;\
    Double_t   lepPtRelJet_PosLep;\
    Double_t   jetPtRelLep_PosLep;\
    Double_t   lepPtRelJetSameLep_PosLep;\
    Double_t   lepPtRelJetOtherLep_PosLep;\
    Double_t   lepJetMt_PosLep;\
    Double_t   lepJetCosTheta_boosted_PosLep;\
    Double_t   lepJetPt_NegLep;\
    Double_t   lepPt_NegLep;\
    Double_t   lepJetDR_NegLep;\
    Double_t   lepJetDPhi_NegLep;\
    Double_t   lepJetDEta_NegLep;\
    Double_t   lepJetM_NegLep;\
    Double_t   lepPtRelJet_NegLep;\
    Double_t   jetPtRelLep_NegLep;\
    Double_t   lepPtRelJetSameLep_NegLep;\
    Double_t   lepPtRelJetOtherLep_NegLep;\
    Double_t   lepJetMt_NegLep;\
    Double_t   lepJetCosTheta_boosted_NegLep;\
    };" );
...
Vars = [['I', 'RunNum'],
        ...
        ['D', 'lepJetDEta_PosLep'],
        ['D', 'lepJetM_PosLep'],
        ['D', 'lepPtRelJet_PosLep'],
        ['D', 'jetPtRelLep_PosLep'],
        ['D', 'lepPtRelJetSameLep_PosLep'],
        ['D', 'lepPtRelJetOtherLep_PosLep'],
        ['D', 'lepJetMt_PosLep'],
        ['D', 'lepJetCosTheta_boosted_PosLep'],
        ['D', 'lepJetPt_NegLep'],
        ['D', 'lepPt_NegLep'],
        ['D', 'lepJetDR_NegLep'],
        ['D', 'lepJetDPhi_NegLep'],
        ['D', 'lepJetDEta_NegLep'],
        ['D', 'lepJetM_NegLep'],
        ['D', 'lepPtRelJet_NegLep'],
        ['D', 'jetPtRelLep_NegLep'],
        ['D', 'lepPtRelJetSameLep_NegLep'],
        ['D', 'lepPtRelJetOtherLep_NegLep'],
        ['D', 'lepJetMt_NegLep'],
        ['D', 'lepJetCosTheta_boosted_NegLep'],
        ]
...
treeList = {'treeZpeak0jets':[TTree('Zpeak0jets','Zpeak0jets'),TTree('Zpeak0jets','Zpeak0jets'),TTree('Zpeak0jets','Zpeak0jets')],
            'treeZpeak1jets':[TTree('Zpeak1jets','Zpeak1jets'),TTree('Zpeak1jets','Zpeak1jets'),TTree('Zpeak1jets','Zpeak1jets')],
            'treeZpeak2jets':[TTree('Zpeak2jets','Zpeak2jets'),TTree('Zpeak2jets','Zpeak2jets'),TTree('Zpeak2jets','Zpeak2jets')],
            'treeZpeakLepSel':[TTree('ZpeakLepSel','ZpeakLepSel'),TTree('ZpeakLepSel','ZpeakLepSel'),TTree('ZpeakLepSel','ZpeakLepSel')],
            }
...
#             leptonSF = lepSF[ModeIdx] - lepSFUnc[ModeIdx]
    ...
    eventInfo.weightC      = extraMultipliers*evtWeightC*evtPUWeightC*TrueLumis[ModeIdx][2]/UsedLumis[2]
    eventInfo.weightNoPUC  = extraMultipliers*evtWeightC*TrueLumis[ModeIdx][2]/UsedLumis[2]

    eventInfo.weightD      = extraMultipliers*evtWeightD*evtPUWeightD*TrueLumis[ModeIdx][3]/UsedLumis[3]
    eventInfo.weightNoPUD  = extraMultipliers*evtWeightD*TrueLumis[ModeIdx][3]/UsedLumis[3]
            
    eventInfo.weightTopPt = topPtweight
...
#    eventInfo.weightBtagSF = btagSF_
    ...
    eventInfo.etajet = jet.Eta()
    eventInfo.etalep0 = lepton0.Eta()
    eventInfo.etalep1 = lepton1.Eta()
    eventInfo.phijet = jet.Phi()
    eventInfo.philep0 = lepton0.Phi()
    eventInfo.philep1 = lepton1.Phi()
    eventInfo.chargelep0 = lep0Charge
    eventInfo.chargelep1 = lep1Charge
    ...
    eventInfo.lepJetPt_PosLep    = (leps[0] + jet).Pt()
    eventInfo.lepPt_PosLep       = leps[0].Pt()
    eventInfo.lepJetDR_PosLep    = abs(leps[0].DeltaR(jet))
    eventInfo.lepJetDPhi_PosLep  = abs(leps[0].DeltaPhi(jet))
    eventInfo.lepJetDEta_PosLep  = abs(leps[0].Eta() - jet.Eta())
    eventInfo.lepJetM_PosLep     = (leps[0] + jet).M()
    eventInfo.lepPtRelJet_PosLep = leps[0].Perp(jet.Vect())
    eventInfo.jetPtRelLep_PosLep = jet.Perp(leps[0].Vect())
    eventInfo.lepPtRelJetSameLep_PosLep = leps[0].Perp((leps[0]+jet).Vect())
    eventInfo.lepPtRelJetOtherLep_PosLep = leps[0].Perp((leps[1]+jet).Vect())
    eventInfo.lepJetMt_PosLep    = (leps[0] + jet).Mt()    
    if len(goodJetIdx) > 0:
        eventInfo.lepJetCosTheta_boosted_PosLep = leps[0].Vect().Dot(p3Jet_)/(leps[0].Vect().Mag()*p3Jet_.Mag())
    else:
        eventInfo.lepJetCosTheta_boosted_PosLep = -2.
        
    eventInfo.lepJetPt_NegLep    = (leps[1] + jet).Pt()
    eventInfo.lepPt_NegLep       = leps[1].Pt()
    eventInfo.lepJetDR_NegLep    = abs(leps[1].DeltaR(jet))
    eventInfo.lepJetDPhi_NegLep  = abs(leps[1].DeltaPhi(jet))
    eventInfo.lepJetDEta_NegLep  = abs(leps[1].Eta() - jet.Eta())
    eventInfo.lepJetM_NegLep     = (leps[1] + jet).M()
    eventInfo.lepPtRelJet_NegLep = leps[1].Perp(jet.Vect())
    eventInfo.jetPtRelLep_NegLep = jet.Perp(leps[1].Vect())
    eventInfo.lepPtRelJetSameLep_NegLep = leps[1].Perp((leps[1]+jet).Vect())
    eventInfo.lepPtRelJetOtherLep_NegLep = leps[1].Perp((leps[0]+jet).Vect())
    eventInfo.lepJetMt_NegLep    = (leps[1] + jet).Mt()    
    if len(goodJetIdx) > 0:
        eventInfo.lepJetCosTheta_boosted_NegLep = leps[1].Vect().Dot(p3Jet_)/(leps[1].Vect().Mag()*p3Jet_.Mag())
    else:
        eventInfo.lepJetCosTheta_boosted_NegLep = -2.


    if not inZpeak and passMET:
        if len(goodJetIdx) == 1:
            if 'tree1jNotagging' in treeList:
                treeList['tree1jNotagging'][ModeIdx].Fill()
            if 'tree1j0t' in treeList:
                if len(btaggedTightJetIdx) == 0:
                    treeList['tree1j0t'][ModeIdx].Fill()
            if 'tree1j1t' in treeList:
                if len(btaggedTightJetIdx) == 1:
                    treeList['tree1j1t'][ModeIdx].Fill()
        if len(goodJetIdx) == 2:
            if 'tree2jNotagging' in treeList:
                treeList['tree2jNotagging'][ModeIdx].Fill()
            if 'tree2j0t' in treeList:
                if len(btaggedTightJetIdx) == 0:
                    treeList['tree2j0t'][ModeIdx].Fill()
            if 'tree2j1t' in treeList:
                if len(btaggedTightJetIdx) == 1:
                    treeList['tree2j1t'][ModeIdx].Fill()
            if 'tree2j2t' in treeList:
                if len(btaggedTightJetIdx) == 2:
                    treeList['tree2j2t'][ModeIdx].Fill()
        if len(goodJetIdx) == 3:
            if 'tree3j0t' in treeList:
                if len(btaggedTightJetIdx) == 0:
                    treeList['tree3j0t'][ModeIdx].Fill()
            if 'tree3j1t' in treeList:
                if len(btaggedTightJetIdx) == 1:
                    treeList['tree3j1t'][ModeIdx].Fill()
            if 'tree3j2t' in treeList:
                if len(btaggedTightJetIdx) == 2:
                    treeList['tree3j2t'][ModeIdx].Fill()
            if 'tree3j3t' in treeList:
                if len(btaggedTightJetIdx) == 3:
                    treeList['tree3j3t'][ModeIdx].Fill()
        if 'tree3plusjNotagging' in treeList:
            if len(goodJetIdx) > 2:
                treeList['tree3plusjNotagging'][ModeIdx].Fill()
        if 'tree3plusj1plustag' in treeList:
            if len(goodJetIdx) > 2:
                if len(btaggedTightJetIdx) > 0:
                    treeList['tree3plusj1plustag'][ModeIdx].Fill()

    if inZpeak:
        if 'treeZpeakLepSel' in treeList:
            treeList['treeZpeakLepSel'][ModeIdx].Fill()
        #No MET cut is applied to 1j1t zpeak region
        if 'tree1j1tZpeak' in treeList:
            if len(goodJetIdx) == 1:
                if len(btaggedTightJetIdx) == 1:
                    treeList['tree1j1tZpeak'][ModeIdx].Fill()
        if 'treeZpeak0jets' in treeList:
            if len(goodJetIdx) == 0:
                treeList['treeZpeak0jets'][ModeIdx].Fill()
        if 'treeZpeak1jets' in treeList:
            if len(goodJetIdx) == 1:
                treeList['treeZpeak1jets'][ModeIdx].Fill()
        if 'treeZpeak2jets' in treeList:
            if len(goodJetIdx) > 1:
                treeList['treeZpeak2jets'][ModeIdx].Fill()
